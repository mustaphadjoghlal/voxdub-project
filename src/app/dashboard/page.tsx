  ...
  const handleAddPackage = async () => {
    ...
    try {
      ...
      setAllPackages(prev => [...prev, {
        id: docRef.id, name: newPkgName.trim(), price: newPkgPrice.trim(),
        desc: newPkgDesc.trim(), features: newPkgFeatures, popular: newPkgPopular,
      }]);
      setNewPkgName(''); setNewPkgPrice(''); setNewPkgDesc('');
      setNewPkgFeatures([]); setNewPkgPopular(false);
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="px-8 py-5 border-b border-gray-100 flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-black text-gray-900">إضافة باقة جديدة</h2>
                </div>
                <div className="p-8 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-black text-gray-700 mb-2">اسم الباقة *</label>
                      <input type="text" value={newPkgName} onChange={e => setNewPkgName(e.target.value)}
                        placeholder="مثال: باقة التعليق الصوتي"
                    ...
                    <div>
                      ...
                      <input type="text" value={newPkgPrice} onChange={e => setNewPkgPrice(e.target.value)}
                  ...
                  <div>
                    ...
                    <input type="text" value={newPkgDesc} onChange={e => setNewPkgDesc(e.target.value)}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-3">
                      <input type="text" value={newPkgFeature} onChange={e => setNewPkgFeature(e.target.value)}
                        onKeyDown={e => e.key === 'Enter' && handleAddFeature()}
                        placeholder="اكتب ميزة واضغط إضافة أو Enter"
                        className="flex-1 px-4 py-3 rounded-xl border border-gray-200 outline-none font-bold text-sm focus:border-red-400 transition" />
                      <button onClick={handleAddFeature} className="bg-gray-900 text-white px-4 py-3 rounded-xl font-black text-sm hover:bg-red-600 transition flex items-center gap-1">
                        <Plus size={16} /> إضافة
                      </button>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => setNewPkgPopular(!newPkgPopular)}
                      className={`w-12 h-6 rounded-full transition-all ${newPkgPopular ? 'bg-red-600' : 'bg-gray-200'} relative`}>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="px-8 py-5 border-b border-gray-100 flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-black text-gray-900">الباقات الحالية ({allPackages.length})</h2>
                </div>
                {allPackages.length === 0 ? (
                  <div className="p-16 text-center">
                    <Tag size={48} className="text-gray-200 mx-auto mb-4" />
                    <p className="text-gray-400 font-black text-lg">لا توجد باقات بعد</p>
                    <p className="text-gray-300 font-bold text-sm mt-2">أضف أول باقة من النموذج أعلاه</p>
                  </div>
                ) : (
                  <div className="divide-y divide-gray-50">
                    {allPackages.map(pkg => (
                          ...
                          <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                              <input value={editingPackage.name} onChange={e => setEditingPackage({ ...editingPackage, name: e.target.value })}
                                className="px-3 py-2 rounded-xl border border-gray-200 outline-none font-bold text-sm focus:border-red-400" placeholder="اسم الباقة" />
                              <input value={editingPackage.price} onChange={e => setEditingPackage({ ...editingPackage, price: e.target.value })}
                                className="px-3 py-2 rounded-xl border border-gray-200 outline-none font-bold text-sm focus:border-red-400" placeholder="السعر" />
                            </div>
                            <input value={editingPackage.desc} onChange={e => setEditingPackage({ ...editingPackage, desc: e.target.value })}
                              className="w-full px-3 py-2 rounded-xl border border-gray-200 outline-none font-bold text-sm focus:border-red-400" placeholder="الوصف" />
                            <div className="space-y-2">
                              {editingPackage.features?.map((f: string, i: number) => (
                                <div key={i} className="flex items-center gap-2">
                                  ...
                                  }} className="flex-1 px-3 py-1.5 rounded-lg border border-gray-200 outline-none font-bold text-sm focus:border-red-400" />
                              ...
                              <button onClick={() => setEditingPackage({ ...editingPackage, features: [...(editingPackage.features || []), ''] })}
                                className="text-gray-500 font-bold text-xs flex items-center gap-1 hover:text-red-600 transition">
                                <Plus size={12} /> إضافة ميزة
                              </button>
                            </div>
                            <div className="flex items-center gap-3">
                              <button onClick={() => setEditingPackage({ ...editingPackage, popular: !editingPackage.popular })}
                                className={`w-10 h-5 rounded-full transition-all ${editingPackage.popular ? 'bg-red-600' : 'bg-gray-200'} relative`}>
                              ...
                              {pkg.features?.length > 0 && (
                                <div className="flex flex-wrap gap-1.5">
                                  {pkg.features.map((f: string, i: number) => (
                                    <span key={i} className="bg-gray-100 text-gray-600 text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1">
            ...
            <div className="glass rounded-2xl overflow-hidden">
              ...
              {artistOrders.length === 0 ? (<div className="px-6 py-12 text-center"><Package size={40} className="text-gray-700 mx-auto mb-3" /><p className="text-gray-500 font-bold">لا توجد طلبات حتى الآن</p></div>) : (
                <div className="divide-y divide-white/5">
                  {artistOrders.slice(0, 6).map((order: any) => { const status = statusConfig[order.status] || statusConfig.pending; return (<div key={order.id} className="px-6 py-4 flex items-center justify-between hover:bg-white/3 transition"><div><p className="text-white font-black text-sm">{order.selectedPackage}</p><p className="text-gray-400 font-bold text-xs mt-0.5">{order.clientName} · {order.workType}</p></div><span className={`px-3 py-1 rounded-full text-xs font-black ${status.color}`}>{status.label}</span></div>); })}
                </div>
              )}
            </div>
            <div className="glass rounded-2xl overflow-hidden">
